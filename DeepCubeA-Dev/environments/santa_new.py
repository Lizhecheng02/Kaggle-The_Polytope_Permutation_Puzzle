...
class Santa(Environment):
    ...
    def __init__(self, num_tiles, num_class, allowed_moves, inverse_allowed_moves, goal_state, all_different = False):
        super().__init__()
        ...
        if all_different:
            self.goal_state = np.array(range(len(goal_state)))
        else:
            self.goal_state = goal_state
    ...
    def random_goal_state(self):
        states_np = self.goal_state
        for _ in range(100):
            move = randrange(self.get_num_moves())
            action = self.allowed_moves_idxs[move]
            states_np = states_np[self.allowed_moves[action]].copy()
        return states_np
        
        
    def generate_goal_states(self, num_states: int, np_format: bool = False) -> Union[List[SantaState], np.ndarray]:
        if np_format:
            states_nps: np.ndarray = [ self.random_goal_state() for _ in range(num_states)]
            states_nps = np.stack(states_nps)
            # goal_np: np.ndarray = np.expand_dims(states_np.copy(), 0)
            # solved_states: np.ndarray = np.repeat(goal_np, num_states, axis=0)
        else:
            raise NotImplementedError
            solved_states: List[SantaState] = [SantaState(states_np.copy(), 0, states_np.copy()) for _ in range(num_states)]
    ...
    def state_to_nnet_input(self, states: List[SantaState]) -> List[np.ndarray]:
        tmp = []
        for state in states:
            tmp_states_np = np.stack([state.state, state.goal_state], axis = 0)
            tmp.append(tmp_states_np)
        states_np = np.stack(tmp)
    ...
    def get_nnet_model(self) -> nn.Module:
        nnet = ResnetModel(self.num_tiles, self.num_class, 5000, 1000, 4, 1, True, self.num_tiles, self.num_class)
    ...
    def generate_states(self, num_states: int, backwards_range: Tuple[int, int]) -> Tuple[List[SantaState], List[int]]:
        ...
        states: List[SantaState] = [SantaState(x,0,y) for x, y in zip(list(states_np), list(goal_states_np))]
    ...
    def expand(self, states: List[State]) -> Tuple[List[List[State]], List[np.ndarray]]:
        ...
        goal_states_np: np.ndarray = np.stack([state.goal_state for state in states])
        ...
        for move_idx in range(num_env_moves):
            ...
            for idx in range(len(states)):
                states_exp[idx].append(SantaState(states_next_np[idx], num_wildcards_np[idx], goal_states_np[idx]))
